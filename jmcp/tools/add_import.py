    ...
    "inputSchema": {
        ...
        "properties": {
            "imports": {
                ...
                "items": {"type": "string"},
                "description": (
                    "List of Python import statements to add, e.g. ['import sys', 'from os.path import join']"
                ),
            },
            "files": {
                "type": "array",
...
def _parse_import_statement(stmt: str) -> list[dict]:
    ...
    if len(tree.body) != 1:
        raise ValueError(
            f"Expected exactly one statement, got {len(tree.body)}: {stmt!r}"
        )
    ...
    if isinstance(imp, cst.Import):
        ...
        for alias in imp.names:
            ...
            asname = (
                tree.code_for_node(alias.asname.name).strip()
                if alias.asname
                else None
            )
            entries.append({
...
def execute(
    imports: list[str], files: list[str], root_path: Path | None = None
) -> str:
    ...
    for path in resolved_files:
        ...
        if modified == source:
            results.append(
                f"{path.relative_to(cwd)}: no changes (imports already present)"
            )
        else:
