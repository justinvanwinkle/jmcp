...
from jmcp.tools.add_import import _add_imports_to_source
from jmcp.tools.add_import import _parse_import_statement
from jmcp.tools.add_import import _resolve_file_patterns
from jmcp.tools.add_import import execute
...
class TestParseImportStatement:
    ...
    def test_dotted_import(self):
        ...
        assert entries == [
            {"module": "os.path", "obj": None, "asname": None, "relative": 0}
        ]
    ...
    def test_import_with_alias(self):
        ...
        assert entries == [
            {"module": "numpy", "obj": None, "asname": "np", "relative": 0}
        ]

    def test_from_import_with_alias(self):
        entries = _parse_import_statement(
...
class TestExecute:
    ...
    def test_regex_targeting(self, tmp_path):
        ...
        result = execute(
            ["import sys"], [r"re:models/.*\.py$"], root_path=tmp_path
        )
        assert "added imports" in result
    ...
    def test_no_files_matched(self, tmp_path):
        result = execute(
            ["import sys"], ["nonexistent/*.py"], root_path=tmp_path
        )
    ...
    def test_invalid_import_string(self, tmp_path):
        ...
        f.write_text("x = 1\n")
        result = execute(
            ["not valid python"], ["target.py"], root_path=tmp_path
        )
